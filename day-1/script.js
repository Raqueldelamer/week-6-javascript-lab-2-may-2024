...
tip = bill * .20;
  // 4. set the message
  messageParagraph.innerHTML = `The total of the $ ${bill} check gets a 20% tip that is $ ${tip} .`
...
function partTwoB () {
 ...
 billNumber = Number(bill);
  ...
  // 4. set the message
  messageParagraph.innerHTML = `Your total bill is $ ${bill}. Your portion of the bill is $ ${split} amongst four friends.`;
}
...
function tipMessage(tip, bill, tipRate) {
  return `Your check total is $${bill}. You should tip $${tip} at ${tipRate}%.`
}
function partThreeB () {
  alert("Try it!");
  let bill, billNumber, tip, message;
  // 1. prompt the user for the bill
  bill = prompt("What is your check total?");
  // 2. convert the bill to a number
  billNumber = Number(bill)
  // 3. calculate the tip
...
function messageForSplitBill(bill, split, count) {
  return `Your check is $${bill}. Your portion of the bill is $${split} amoung ${count} friends.`
}
...
function partFourB () {
  ...
  let bill, billNumber, split, message;
  // 1. prompt for bill
bill = prompt("What is the check total?");
  // 2. convert bill to number 
billNumber = Number(bill);
  // 3. use a function to calculate the split
